import { TierType, ModifierType } from "@/app/gear/page";

export const getModifierWeight = (modifierGroup: string, excludingModifiers: string[], availableTiers: TierType[]) => {
...
export const getTierDisplayForModifier = (
    ...
    if (tier.value.min !== undefined || tier.value.minChance !== undefined) {
        ...
        if (
            (Number.isInteger(tier.value.min) &&
                Number.isInteger(tier.value.max)) ||
...
export const getModifierDisplayForTier = (tier: TierType, langData: {
    name: string;
    description: string;
    color: string;
}) => {
